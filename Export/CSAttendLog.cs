...
{
    public partial class CSAttendLog : EMBA.Export.ExportProxyForm
    {
        ...
        {
            InitializeComponent();
            base.TitleText = "匯出資料至Excel-未選上學生";
            base.Text = "匯出資料至Excel-未選上學生";
            this.Text = "匯出資料至Excel-未選上學生";
            this.TitleText = "匯出資料至Excel-未選上學生";
...
where student.id || '-' || ref_course_id in 
(
select ref_student_id || '-' || ref_course_id from $ischool.emba.cs_attend_log
where action='delete' and action_by='staff' and school_year={0} and semester={1} {2}
) 
and  student.id || '-' || ref_course_id not in 
(
select ref_student_id || '-' || ref_course_id from $ischool.emba.cs_attend where school_year={0} and semester={1} {2}
)
and student.id || '-' || ref_course_id in 
(
select ref_student_id || '-' || ref_course_id from $ischool.emba.cs_attend_log
where action='insert' and action_by='student' and school_year={0} and semester={1} {2} 
) 
and course.school_year={0} and course.semester={1} and cl.action='delete' and cl.action_by='staff' {2}
group by 學生系統編號, 課程系統編號, 學年度, 學期, 開課, 學分, 授課教師, 年級, 入學年度, 組別, 班次, 學號, 學生姓名
order by course.course_name, sb.grade_year, sb.enroll_year, dg.name, class_name, student_number", school_year, semester, (item == 1 ? " and item=1" : ""));
...
order by course.id", String.Join(",", dataTable.Rows.Cast<DataRow>().Select(x => x["課程系統編號"] + ""))));
        ...
        protected override void OnExportButtonClick(object sender, EventArgs e)
        {
            ...
            task.ContinueWith((x) =>
            {
                ...
                sfd.Title = "另存新檔";
                sfd.FileName = school_year + "學年度" + DataItems.SemesterItem.GetSemesterByCode(semester.ToString()).Name + (item == 1 ? "第一階段" : "第二階段") + "未選上學生名單.xls";
